...
const TopNav = ({ userName = "Keshav" }) => {
    const { user } = useSelector((state) => state.auth)
    console.log(user)
    return (
        <header className="h-15 w-full flex items-center justify-between px-10 border-b border-purple-100/40 bg-white/80 backdrop-blur-md sticky top-0 z-20">
            ...
            <div className="flex items-center gap-5">
                ...
                <button className="flex items-center gap-2 bg-[#F4F2FA] text-[#4A4765] px-3.5 py-1.5 rounded-full text-xs font-semibold hover:bg-purple-100/60 transition-colors">
                    ...
                    <span>{user.generationCredits} credits</span>
                </button>
